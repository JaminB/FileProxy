...
urlpatterns = [
    ...
    path("users/", lambda req: HttpResponsePermanentRedirect(
        "/admin-panel/users/?" + req.META.get("QUERY_STRING", "") if req.META.get("QUERY_STRING") else "/admin-panel/users/"
    )),
    path("users/<int:user_id>/", lambda req, user_id: HttpResponsePermanentRedirect(
        f"/admin-panel/users/{user_id}/?" + req.META.get("QUERY_STRING", "") if req.META.get("QUERY_STRING") else f"/admin-panel/users/{user_id}/"
    )),
