...
async function load(days: number = DEFAULT_DAYS): Promise<void> {
  ...
  try {
    const [summary, timeline] = await Promise.all([
      apiJson<Summary>(
        `/api/v1/usage/summary/?connection=${encodeURIComponent(connectionName)}&days=${days}`,
      ),
      apiJson<Timeline>(
        `/api/v1/usage/timeline/?connection=${encodeURIComponent(connectionName)}&days=${days}`,
      ),
    ]);
