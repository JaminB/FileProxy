...
function getItemId(): string {
  ...
  if (!el) throw new Error('Missing connection-id script tag');
...
function render(item: ConnectionDetail): void {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let current: ConnectionDetail | null = null;

  async function load(): Promise<void> {
    try {
      const item = await api<ConnectionDetail>(`/api/v1/connections/${itemId}/`);
      current = item;
      render(item);
    } catch (e) {
  ...
  btnTest.addEventListener('click', async () => {
    ...
    try {
      const out = await api<{ ok: boolean; message: string; details?: unknown }>(
        `/api/v1/connections/${itemId}/test/`,
        { method: 'POST' },
      );
      if (out.ok) {
