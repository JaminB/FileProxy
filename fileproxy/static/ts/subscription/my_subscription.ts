...
function renderSubInfo(sub: Sub, subInfoEl: HTMLElement): void {
  ...
  cancelBtn?.addEventListener("click", async () => {
    if (!confirm("Cancel your subscription? You will remain on your current plan until the end of the billing cycle.")) return;
    try {
    ...
    plansListEl.querySelectorAll<HTMLButtonElement>(".switch-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (modal) {
            const bsModal = (window as typeof window & { bootstrap?: { Modal?: { getInstance?: (el: HTMLElement) => { hide(): void } | null } } }).bootstrap?.Modal?.getInstance?.(modal);
            bsModal?.hide();
          }
