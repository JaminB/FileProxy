...
function renderSubInfo(sub, subInfoEl) {
  ...
  `;
    ...
    cancelBtn?.addEventListener("click", async () => {
        if (!confirm("Cancel your subscription? You will remain on your current plan until the end of the billing cycle."))
            return;
        try {
            ...
            setFlash("Subscription canceled. You will remain on your plan until the end of the cycle.", "info");
            await loadAll();
        }
        catch (err) {
...
function renderUsageBars(usage, usageBarsEl) {
...
async function loadAvailablePlans(plansListEl) {
    try {
        ...
        plansListEl.querySelectorAll(".switch-btn").forEach((btn) => {
            btn.addEventListener("click", async () => {
                ...
                try {
                    ...
                    if (!resp.ok)
                        throw new Error(`Switch failed (${resp.status})`);
...
async function loadAll() {
    const subInfoEl = qs("#sub-info");
    const usageBarsEl = qs("#usage-bars");
    const plansListEl = qs("#available-plans-list");
    if (!subInfoEl || !usageBarsEl)
        return;
    try {
        const [subResp, usageResp] = await Promise.all([
            fetch("/api/v1/subscription/my/", { headers: { Accept: "application/json" }, credentials: "same-origin" }),
            fetch("/api/v1/subscription/my/usage/", { headers: { Accept: "application/json" }, credentials: "same-origin" }),
        ]);
        if (!subResp.ok) {
...
//# sourceMappingURL=my_subscription.js.map