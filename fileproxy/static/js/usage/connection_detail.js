...
async function load(days = DEFAULT_DAYS) {
    const nameEl = document.getElementById('connection-name-data');
    const connectionName = nameEl?.dataset['connectionName'] ?? '';
    if (!connectionName)
        return;
    renderPeriodSelector(days);
    try {
        const [summary, timeline] = await Promise.all([
            apiJson(`/api/v1/usage/summary/?connection=${encodeURIComponent(connectionName)}&days=${days}`),
            apiJson(`/api/v1/usage/timeline/?connection=${encodeURIComponent(connectionName)}&days=${days}`),
        ]);
...
//# sourceMappingURL=connection_detail.js.map