...
async function load(days = DEFAULT_DAYS) {
    ...
    try {
        const [summary, timeline] = await Promise.all([
            apiJson(`/api/v1/usage/summary/?vault=${encodeURIComponent(vaultName)}&days=${days}`),
            apiJson(`/api/v1/usage/timeline/?vault=${encodeURIComponent(vaultName)}&days=${days}`),
        ]);
...
//# sourceMappingURL=vault_detail.js.map